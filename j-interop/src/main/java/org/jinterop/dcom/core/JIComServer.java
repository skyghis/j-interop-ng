 ...
 * <code>
 *
 *  {@link JISession} session =
 * JISession.createSession("DOMAIN","USERNAME","PASSWORD"); <br>
 * JIComServer excelServer = new
 * JIComServer(JIProgId.valueOf("Excel.Application"),address,session); <br>
 * IJIComObject comObject = excelServer.createInstance(); <br>
 *  //Obtaining the IJIDispatch (if supported) <br> {@link IJIDispatch} dispatch
 * =
 * (IJIDispatch){@link JIObjectFactory}.narrowObject(comObject.queryInterface(IJIDispatch.IID));
 * <br>
 * </code>
 *
 * <p>
...
//		rpc.ntlm.keyExchange = false
     ...
     * </p>
     *
     * @param session Please use a new session and not an already bounded one.
...
//		}
     ...
     * @throws JIException will <i>also</i> get thrown in case the
     * <code>session</code> is associated with another server already.
     * @throws IllegalArgumentException raised when either <code>progId</code>
     * or <code>session</code> is <code>null</code>.
     ...
     *
     * @param clsid 128 bit string such as
     * "00024500-0000-0000-C000-000000000046".
     * @param session session to be associated with.
     * @throws JIException will <i>also</i> get thrown in case the
     * <code>session</code> is associated with another server already.
     * @throws IllegalArgumentException raised when either <code>clsid</code> or
     ...
     * @param progId user-friendly string such as "Excel.Application" ,
     * "TestCOMServer.Test123" etc.
     * @param address address of the host where the <code>COM</code> object
     * resides.This should be in the IEEE IP format (e.g. 192.168.170.6) or a
     * resolvable HostName.
     * @param session session to be associated with.
     * @throws JIException will <i>also</i> get thrown in case the
     * <code>session</code> is associated with another server already.
     * @throws IllegalArgumentException raised when any of the parameters is
     * <code>null</code>.
     * @throws UnknownHostException
     */
